...
def write_hw_list(input_list):
    myfile = open("HW008/hw.txt", "a", encoding="utf-8") 
    for s in input_list:
        if ";" not in s:
            continue
        myfile.write(s) 
...
def add_hw(): 
    d = my.file_to_dct("HW008/hw.txt", "utf-8", ";")
    n = 0
    for key in d:
        if int(key) > n:
            n = int(key)
...
def display_scores_list(input_list: list):
    ...
    hw_dct = my.file_to_dct("HW008/hw.txt", "utf-8", ";")
...
def get_debts():
    hw_dct = my.file_to_dct("HW008/hw.txt", "utf-8", ";")
    users_dct = my.file_to_dct("HW008/users.txt", "utf-8", ";")
    students_dct = {}
    ...
    result = []
    scores_file = open("HW008/journal.txt", "r", encoding="utf-8") 
...
def put_rating():
    ...
    myfile = open("HW008/journal.txt", "a", encoding="utf-8") 
    myfile.write(s) 
    myfile.close()     
